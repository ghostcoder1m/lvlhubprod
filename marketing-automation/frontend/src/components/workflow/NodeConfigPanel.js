...
const NodeConfigPanel = ({ node, onUpdate, onDelete }) => {
    ...
    <div className="space-y-4">
      <div>
        ...
        >
          ...
          {configData.triggers?.map((trigger) => (
      ...
      {configData.triggerType === 'Scheduled' && (
        <div>
          ...
          <select
            value={configData.scheduleType || 'once'}
          ...
          {configData.scheduleType === 'recurring' && (
          ...
          {configData.scheduleType === 'specific_days' && (
            <div className="mt-2 space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
                  <label key={day} className="inline-flex items-center">
                    <input
                      type="checkbox"
                      checked={configData.selectedDays?.includes(day) || false}
                      onChange={(e) => {
                        const days = configData.selectedDays || [];
  ...
  const renderActionConfig = () => (
    <div className="space-y-4">
      ...
      {configData.actionType === 'Send Email' && (
        <div className="space-y-2">
          <div>
      ...
      {configData.actionType === 'Send SMS' && (
        <div className="space-y-2">
          <div>
            <label className="block text-sm font-medium text-gray-700">Template</label>
            <select
      ...
      {configData.actionType === 'Create Task' && (
        <div className="space-y-2">
          <div>
  ...
  const renderConditionConfig = () => (
    <div className="space-y-4">
      <div>
        ...
        <select
          value={configData.conditionType || ''}
          onChange={(e) => handleConfigChange('conditionType', e.target.value)}
          className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"
        >
          <option value="">Select Condition Type</option>
          {configData.conditions?.map((condition) => (
            <option key={condition} value={condition}>{condition}</option>
          ))}
        </select>
      </div>

      {configData.conditionType === 'Contact Field' && (
        <div className="space-y-2">
          <div>
      ...
      {configData.conditionType === 'Tag Exists' && (
        <div>
  ...
  const renderDelayConfig = () => (
    <div className="space-y-4">
      ...
      {configData.delayType === 'Until Time of Day' && (
        <div>
          <label className="block text-sm font-medium text-gray-700">Time</label>
          <input
            type="time"
  ...
  const renderIntegrationConfig = () => (
    <div className="space-y-4">
      ...
      {configData.integrationType === 'Custom Webhook' && (
        <div className="space-y-2">
          <div>
            <label className="block text-sm font-medium text-gray-700">Webhook URL</label>
            <input
